...
class TicketListFromStatus extends StatefulWidget {
  final String status;


  TicketListFromStatus({
    Key? key,
    required this.status,
  }) : super(key: key);

  @override
  _TicketListFromStatusState createState() => _TicketListFromStatusState(
    status: status,
  );
...
class _TicketListFromStatusState extends State<TicketListFromStatus> {
  ...
  Future complainList() async {
    ...
    final response =
    await http.post(Uri.parse(ticket_status_list), body: queryParameters);
    print(response.body);
